...
class SecureCCChecker:
    ...
    def generate_card_number(self, bin_num):
        ...
        # Añadir dígito Luhn y verificar longitud
        final_number = self.luhn_complete(number)
        
        # Verificación de seguridad
        if len(final_number) != 16:
            print(f"{Fore.YELLOW}⚠️  Advertencia: Número generado con {len(final_number)} dígitos")
    ...
    def show_generated_cards(self):
        ...
        for i, card in enumerate(self.generated_cards, 1):
            ...
            if len(card['number']) != 16:
                length_status = f" [{len(card['number'])} dígitos]"
            
            print(f"{i}. {color}{card['number']} | {card['exp_month']}/{card['exp_year']} | {card['cvc']} | {card['card_type']} | {status}{length_status}")
